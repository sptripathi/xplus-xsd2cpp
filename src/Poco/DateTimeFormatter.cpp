      ...
      if (*it == '%')
      {
        if (++it != end)
        {
          switch (*it)
          {
            ...
            case 'S': NumberFormatter::append0(str, (short)(dateTime.second()), 2); break;
            case 's': 
                      {
                      ...
                      break;
            case 'z': appendTzdISO(str, dateTime, timeZoneDifferential); break;
...
void DateTimeFormatter::append(std::string& str, const Duration& duration)
{
  std::string fmt = DateTimeFormat::XSD_DURATION_FORMAT;
  std::string::const_iterator it  = fmt.begin();
  std::string::const_iterator end = fmt.end();
  while (it != end)
  {
...
//xplus
void DateTimeFormatter::appendTzdISO(std::string& str, DateTime dateTime, int timeZoneDifferential)
{
  int tzd = DateTime::UNKNOWN_TZD; 
  if( (timeZoneDifferential != DateTime::UNKNOWN_TZD) && dateTime.tzAvailable() )
  {
    if(dateTime.tzd() != timeZoneDifferential) {
      throw TZException("timezone differential does not match with timezone availabe with the object");
    }
  }
  else if(dateTime.tzAvailable())
  {
    tzd = dateTime.tzd();
  }
  else if(timeZoneDifferential != DateTime::UNKNOWN_TZD)
  {
    tzd = timeZoneDifferential;
  }
  else
  {
    tzd = DateTime::UNKNOWN_TZD;
  }
  tzdISO(str, tzd);
}
...
{
  if (timeZoneDifferential == DateTime::UNKNOWN_TZD)
  {
    return;
  }
  else if (timeZoneDifferential == 0) //UTC
  {
    str += 'Z';
  }
  else if (timeZoneDifferential > 0)
  {
    str += '+';
