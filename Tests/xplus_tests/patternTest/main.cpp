...
{
  // *Warning*:
  // These assert are put assuming the input xml document is valid.xml
  // which is supplied alongwith this dir.
  // If the input changes these asserts would fail...
  assert(xsdDoc->element_root()->get_attr_sku_string() == "123-AB");
  assert(xsdDoc->element_root()->get_code_string(0) == "12345");
  assert(xsdDoc->element_root()->get_code_string(1) == "abcde");
  assert(xsdDoc->element_root()->get_sku_string() == "987-EF");
}
